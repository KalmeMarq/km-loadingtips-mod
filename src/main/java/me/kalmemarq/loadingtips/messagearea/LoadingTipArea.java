...
public class LoadingTipArea extends LoadingMessageArea {
    public static ILoadingMessageAreaSerializer<LoadingMessageArea> SERIALIZER = new Serializer();

    protected Anchor anchorFrom = Anchor.BOTTOM_LEFT;
    protected Anchor anchorTo = Anchor.BOTTOM_LEFT;

    public LoadingTipArea(@Nullable Integer spaceGap) {
        this(spaceGap, new ArrayList<>(), new ArrayList<>(), new ArrayList<>());
    }

    public LoadingTipArea(@Nullable Integer spaceGap, List<ILoadingMessage> messages, List<String> visible, List<String> ignored) {
        super(spaceGap, messages, visible, ignored);
    }

    private static final class Serializer implements ILoadingMessageAreaSerializer<LoadingMessageArea> {
        public LoadingMessageArea fromJson(JsonObject obj) {
            ...
            if (JsonHelper.hasJsonObject(obj, "tip_area")) {
                JsonObject areaObj = JsonHelper.getObject(obj, "tip_area");
            ...
            if (JsonHelper.hasArray(obj, "tips")) {
                JsonArray tipsArr = JsonHelper.getArray(obj, "tips");

                for (JsonElement item : tipsArr) {
                    if (JsonHelper.isString(item)) {
                        list.add(new LoadingTip(LoadingMessage.DEFAULT_TIP_TITLE, Text.translatable(item.getAsString()), LoadingMessage.DEFAULT_STAY_TIME));
                    } else if (item.isJsonObject()) {
                        JsonObject itemObj = item.getAsJsonObject();

                        LoadingTip msg = (LoadingTip)LoadingTipsMod.getMsgSerializer(LoadingMessageType.TIP).fromJson(itemObj);
            ...
            LoadingTipArea tipArea = new LoadingTipArea(spaceGap, list, vis, ig);
            tipArea.setAnchor(ancFrom, ancTo);
            tipArea.setTextAlignment(align);
            tipArea.setOffset(off);
            tipArea.setSize(sz);
            return tipArea;
        }
    }
